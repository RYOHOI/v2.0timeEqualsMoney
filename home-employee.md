...
--------------------
注：抢单兼职当前仅在上海（待定）开通。如 v2.0 App 定位发现用户所处城市未开通抢单兼职，则移除「抢单」「兼职快讯」tab，直接显示「兼职快讯」内容。
...
--------------------
注：身份证认证服务：http://www.id98.cn/doc/idcard；账号：359733246@qq.com；密码：chen112233
...
```
认证未完成
预计审核时长 6 小时左右，请您稍候重试！（首次抢单机会用光了，完成认证可继续抢单）
```
...
- 岗位图标
- 距离：仅选中「离我最近」筛选项时显示（格式：310m 或 1.3km）
...
## 搜索
沿用旧版本功能设定，搜索建议项分为「专题」「分类」两栏。

![image](img/search.png)

搜索范围包括**抢单**及普通兼职。列表项中包含「标题」「抢单标识」「工作地点」「工作时间」「薪资」「暂无名额印章」「专题标识」「置顶标识」等。
