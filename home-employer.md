...
- 报名中的岗位（普通/抢单）：为最常见状态
- [已结束的岗位](id:finished)：用背景色强调
	- 主动结束：雇主通过「结束招聘」
	- 自动结束：岗位到期，即工作结束时间点已过，可能已招满或未招满。系统自动拒绝所有未处理兼客，该岗位挪到「已完成」中。
- 审核中的岗位：稍作置灰处理
- 被举报：被客服强制关闭，则系统立即自动拒绝所有已报名兼客，该岗位转到「已完成」tab 中。

--------------------
注：可由 UI 直接表现的状态仅报名中、主动结束、审核中 3 种，自动结束及被举报 2 种状态留存时间极短，直接被转入「已完成」 tab 中。
...
- 刷新岗位：沿用旧版本功能设定
- 结束招聘：点击后弹框确认「结束招聘后岗位将被关闭，但不影响录用已报名兼客。」「取消」「确认结束」
...
- 处理完成：最常见状态。点击后跳转至「兼客管理」
- 被举报：等于旧版本「已下架」。点击后直接跳转至「岗位详情」。
- 审核失败：点击后直接跳转至「岗位详情」。
...
#### 兼客名片
点击列表项可进入「兼客名片」，兼客名片标题栏含可选动作「电话联系」及「IM 对聊」。
